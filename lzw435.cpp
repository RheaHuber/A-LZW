// lzw435.cpp
// Compresses a given filename using 12-bit LZW
...
int main(int argc, char *argv[]) {
   ...
   if(*argv[1] == 'c' || *argv[1] == 'C') {
      ...
      std::ofstream output(fileName + ".lzw", std::ios::trunc);
      ...
      for(std::vector<int>::const_iterator iter = compressed.begin(); iter != compressed.end(); ++iter) {
         binaryOutput += int2BinaryString(*iter, DICTMAXBITS);
         bitCount = (bitCount + DICTMAXBITS) % 8; // Track # of trailing bits
      }
   ...
   if(*argv[1] == 'e' || *argv[1] == 'E') {
      ...
      std::vector<int> compressed;
      for(int start = 0; (inputBinary.size() - start) > DICTMAXBITS; start += DICTMAXBITS) {
         compressed.push_back(binaryString2Int(inputBinary.substr(start, DICTMAXBITS)));
      }

      // Create file to output decompressed data
      std::ofstream outputFile(fileName.substr(0, (fileName.size() - 4)) + "2", std::ios::trunc);
